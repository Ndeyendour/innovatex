...
function simpleMap(latitude, longitude, markerImage, mapStyle, mapElement, markerDrag){
  ...
  document.addEventListener("DOMContentLoaded", () => {
    ...
    const texts = Array.from(headings).map(h => h.getAttribute("data-text"));

    // On n'utilise qu'un seul <h4> pour afficher les textes tour à tour
    ...
    function typeText(text, onComplete) {
      ...
      function type() {
        if (i < text.length) {
          ...
          setTimeout(onComplete, 1500); // pause avant l'effacement
        }
      }
      type();
    }
    ...
    function loopAnimation() {
      ...
      typeText(text, () => {
        index = (index + 1) % texts.length; // passe au suivant, boucle
        loopAnimation();
      });
    }
